...
Loads one hand-picked token that has several distinct routing options (left / straight / right at
a nearby intersection) and writes one figure per route — the original route plus each alternative
...
def test_visualize_routes():
    out_dir = os.environ.get("NUCONTROL_VIZ_OUT", str(OUTPUT_DIR / "route_viz"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("out_dir", nargs="?", default=str(OUTPUT_DIR / "route_viz"))
    parser.add_argument(
        "--config",
        default=None,
